...
def start_game(player_id):
    ...
    if player_score == 21 and dealer_score == 21:
        ...
        return f"😐 平局！你和庄家都拿到了 Blackjack！\n你的手牌: {', '.join(player_hand)}（21 点）\n庄家的手牌: {', '.join(dealer_hand)}（21 点）\n输入 'start' 重新开始游戏。"
    ...
    if dealer_score == 21:
        redis_client.hset(player_id, "game_over", "True")
        return f"😢 庄家拿到了 Blackjack！\n你的手牌: {', '.join(player_hand)}（{player_score} 点）\n庄家的手牌: {', '.join(dealer_hand)}（21 点）\n输入 'start' 重新开始游戏。"
    ...
    return f"🎰 你拿到的牌: {', '.join(player_hand)}（{calculate_hand_value(player_hand)} 点）\n" \
...
def hit_card(player_id):
    ...
    return f"🃏 你拿到了 {', '.join(player_hand)}（{player_score} 点）\n输入 'hit'（要牌） 或 'stand'（停牌）。"
...
def stand(player_id):
    ...
    return f"🃏 你的手牌: {', '.join(player_hand)}（{player_score} 点）\n" \
